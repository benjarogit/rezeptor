...
### Install-Orchestrierung (ein Pfad)

| Pfad | Status | Nutzung |
|------|--------|---------|
| `recipes/<id>/*.sh` + `recipe_hooks::*` | **Aktuell** | Alle neuen und gepflegten Rezepte |
| `launcher()` in `core/sharedFuncs.sh` | **LEGACY** | Nur historischer Photoshop-CLI-Desktop-Deploy — keine neuen Aufrufer |
| `photoshop::*`-Helfer in `sharedFuncs.sh` | Geteilte Utilities | Von modernem `recipes/photoshop/*` bei Bedarf |

Updates laufen über `scripts/rezeptor-update.sh` (kein zweites `core/update.sh`).
...
| `recipe_hooks::purge_recipe_data` | Desktop + gewählter + kanonischer `data_root` + Asset-Cache `cache/<id>/` / `cache/<id>-mod-bundle/` |
...
| `recipe_winetricks::sanitize_win7sp1_cache` | Kaputte/zu kleine `win7sp1`-Downloads entfernen bzw. Hash-Datei → kanonischer Name |
| `recipe_winetricks::purge_win7sp1_cache` | Win7-SP1-Datei(en) im Cache löschen (vor MSXML/IE8-Retry) |
...
- Adobe MSXML/IE8 (`adobe_setup::ensure_native_msxml` / `install_ie8`): vor dem Lauf `sanitize_win7sp1_cache`; bei Fehlschlag Purge + einmal Retry (kein blindes `--force` auf kaputtem Cache)
...
**Schritt-Typen:** `prepare_source`, `require_portable`, `prefix`, `winetricks`, `deploy_graphics`, `run_installer`, `apply_updates`, `stabilize_prefix`, `win10`, `fonts_registry`, `emit_log_paths`, `module`, `copy_asset`, `env_set`, `progress`, `vcrun`, `dotnet`.
...
Siehe [UPDATES.md](UPDATES.md). Kurz:
...
| `recipe-assets.sh` | `stage_pack` / `fetch_pack` / `ensure` — Cache `cache/<rezept-id>/`, SHA-256; `discard_consumed_archive` / `discard_yml_archives` nach Erfolg; `purge_recipe_cache` bei Uninstall |
