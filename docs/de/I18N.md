...
Quelle der Wahrheit: `msg::t` / `msg::_lookup` in `core/i18n.sh` (festes `case` für DE/EN, gesteuert über `RECIPE_UI_LANG`).  
Es gibt **keine** externen `.lang`-Dateien mehr (Legacy-Bäume `core/locales/` und `scripts/locales/` entfernt).  
Launcher-JSON und Shell-`msg::t` teilen **keine** Dateien — getrennte Systeme.
