...
Currently **launcher** JSON only (~762 keys today). Not covered: `core/locales/*.lang`, `scripts/locales/*.lang`, recipe `info.*.txt`.
...
`make i18n-check` does **not** cover these `.lang` trees — keep them in sync manually / via review.
