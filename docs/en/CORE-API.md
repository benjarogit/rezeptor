...
| `recipe_iso::mount` / `ensure_accessible` | Mount large game ISOs (no full extract); mount point = `RECIPE_ISO_MOUNT` (not `$()`) |

### Relocate target — `recipe-relocate.sh`

| Function | Role |
|----------|------|
| `recipe_relocate::move` | `DATA_ROOT` → `RECIPE_RELOCATE_TO`; `data_root.path`; dosdevices; desktop refresh |

CLI: `scripts/recipe-relocate.sh <recipe_dir>` with env `RECIPE_RELOCATE_TO`.
...
Pin: `core/runtime.lock` (default). Recipe override: `proton_ge_tag:` (+ optional URL/SHA) or env `PROTON_GE_TAG` before `wine_runtime::init` — `_load_lock` keeps set tags and resolves ALT pins. System Wine exists in code only with explicit `runtime: system` / env — **forbidden** in recipes.
...
| Function | Role |
|----------|------|
| `recipe_deploy::sync_portable <src> <dst> <mode>` | `copy` (default), `move`, `link` (`inplace` = legacy alias for `link`) |
| `recipe_deploy::detect_installer <dir>` | Find setup / largest EXE |

Schema/`recipe.yml`: `deploy_mode: copy|link|move`. Typical portable: `copy` or `link` (Steam).
...
App-specific: `recipe-photoshop-*.sh`, `recipe-premiere-*.sh`, `recipe-adobe-setup.sh` (shared Adobe ESD), `recipe-wiso-steuer.sh`, …
