...
# Pfade — core + unified data root
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
CORE_DIR="$PROJECT_ROOT/core"
export PROJECT_ROOT CORE_DIR

if [ -f "$CORE_DIR/sharedFuncs.sh" ]; then
    # shellcheck source=/dev/null
    source "$CORE_DIR/sharedFuncs.sh"
fi
if [ -f "$CORE_DIR/paths.sh" ]; then
    # shellcheck source=/dev/null
    source "$CORE_DIR/paths.sh"
fi
if [ -f "$CORE_DIR/wine-runtime.sh" ]; then
    # shellcheck source=/dev/null
    source "$CORE_DIR/wine-runtime.sh"
fi

SCR_PATH="${SCR_PATH:-$(recipe_data_root photoshop 2>/dev/null || echo "$HOME/.local/share/wine-software/photoshop")}"
if [ -f "$HOME/.psdata.txt" ]; then
    _legacy="$(head -n 1 "$HOME/.psdata.txt" 2>/dev/null || true)"
    if [ -n "$_legacy" ] && [ -d "$_legacy" ]; then
        SCR_PATH="$_legacy"
    fi
fi
