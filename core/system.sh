...
system::is_immutable() {
    ...
    if [ -f /etc/os-release ]; then
        ...
        . /etc/os-release
        case "${ID:-}:${VARIANT_ID:-}:${ID_LIKE:-}" in
            *bazzite*|*bluefin*|*ublue*|*aurora*|*silverblue*|*kinoite*|*sericea*)
                return 0
                ;;
        esac
    fi
    if [ -d /usr ] && ! touch /usr/.photoshop-immutable-test 2>/dev/null; then
        return 0
    fi
    rm -f /usr/.photoshop-immutable-test 2>/dev/null || true
    return 1
}
