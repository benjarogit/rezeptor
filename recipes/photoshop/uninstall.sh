...
main() {    
    ...
    log_debug "Loading installation paths..."
    ...
    if [ -n "${PROTON_PATH:-}" ]; then
        WINE_VERSION_INFO="$PROTON_PATH"
        export WINE_VERSION_INFO
    fi
    if [ -f "$HOME/.psdata.txt" ]; then
        WINE_VERSION_INFO="$(paths_read_psdata_proton_path 2>/dev/null || true)"
        _legacy="$(head -n 1 "$HOME/.psdata.txt" 2>/dev/null || true)"
        if [ ! -d "$SCR_PATH/prefix" ] && [ -n "$_legacy" ] && [ -d "$_legacy/prefix" ]; then
            SCR_PATH="$_legacy"
            log_warning "Legacy SCR_PATH aus ~/.psdata.txt: $SCR_PATH"
        fi
    fi
    ...
    result=""
    if [ "${PHOTOSHOP_UNINSTALL_YES:-0}" = "1" ] || [ "${UNINSTALL_YES:-0}" = "1" ]; then
        result="yes"
    else
...
# Load paths only when legacy psdata exists (recipe DATA_ROOT is set via recipe_export_env)
if [ -f "$HOME/.psdata.txt" ]; then
    load_paths "true"
fi
