...
output::step "Pack-Extras (Neural Filters / missing_libs, falls Pack bekannt)"
recipe_hooks::_source recipe-photoshop-pack.sh
recipe_photoshop_pack::apply_extras >> "$LOG_FILE" 2>&1 || true

output::step "GenP (Medizin: nur wenn an)"
recipe_photoshop_pack::maybe_run_genp_on_repair >> "$LOG_FILE" 2>&1 || true
